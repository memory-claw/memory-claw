...
STEPS = [
    ...
    "12. Verify Slack receives the RFP message and audit_log.jsonl has draft_read path=inbox/new_rfp_draft.txt, driver=openclaw, count > 0, source=corpus/2023_rfp_postmortem.txt, source_attributions includes corpus/2023_rfp_postmortem.txt, slack_sent status=sent, processed path=inbox/new_rfp_draft.txt status=sent.",
    ...
    "16. Verify no Slack message posts and audit_log.jsonl has draft_read path=inbox/000_silent_clinical_trial_protocol.txt, driver=openclaw, memory_searched count: 0, no slack_sent before processed, and processed path=inbox/000_silent_clinical_trial_protocol.txt status=skipped_no_relevant_memory.",
