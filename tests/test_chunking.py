...
def test_chunk_overlap_present(monkeypatch):
    ...
    chunks = ingest.chunk_text(text, "company/corpus/source.txt")
...
def test_chunk_ids_include_source_and_offset(monkeypatch):
    monkeypatch.setattr(ingest, "CHUNK_SIZE", 20)
    monkeypatch.setattr(ingest, "CHUNK_OVERLAP", 5)
    chunks = ingest.chunk_text(" ".join(f"word{i}" for i in range(30)), "company/corpus/source.txt")
    assert chunks[0]["id"].startswith("company/corpus/source.txt:0:")
